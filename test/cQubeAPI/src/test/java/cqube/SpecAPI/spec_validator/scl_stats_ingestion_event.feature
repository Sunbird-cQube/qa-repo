...
Feature: Validate the School stats Ingestion Event API

@setup=project
@ignore
Scenario:
    * def actual_request = read('classpath:src/test/java/cqube/SpecAPI/spec_validator/scl_stats_ingestion_event.json')

Scenario Outline: ${__row.scenario}

    Given url "https://cqube-qa.tibilprojects.com"
    Given path '/ingestion/event'
