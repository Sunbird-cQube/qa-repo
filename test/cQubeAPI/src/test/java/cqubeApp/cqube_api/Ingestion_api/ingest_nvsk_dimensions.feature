...
Background: Defining the App domain and API end point 
    * def tokenvalue = call read('classpath:/src/test/java/cqube/Ingest_ImportCsv/Programs/get_jwt_token.feature')
...
Scenario Outline: Uploading the dimension files to buckets 
    * def files = "../Request_Data/NVSK_Dimensions/<file>"
    * print files
    * def app_url = call read('classpath:/src/test/java/cqube/app_url.feature')
    * def URL = app_url.app_url
    ...
    * if (responseStatus == 500 || responseStatus == 501 || responseStatus == 502 || responseStatus == 503 || responseStatus == 504 ) karate.fail('Server error')
        ...
        Examples:
        |file|
        |block-dimension.data.csv|
        |cluster-dimension.data.csv|
        |district-dimension.data.csv|
        |exam-dimension.data.csv|
        |grade-dimension.data.csv|
        |medium-dimension.data.csv|
        |school-dimension.data.csv|
        |statecategory-dimension.data.csv|
