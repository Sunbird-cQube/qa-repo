...
Scenario Outline: Ingestion of event files 
    * def request_data = read("classpath:/src/test/java/cqube/Programs_SpecAPI/Grammers/NVSK/<file>")
    * print request_data 
    ...
    * if (responseStatus == 500 || responseStatus == 501 || responseStatus == 502 || responseStatus == 503 || responseStatus == 504 ) karate.fail('Server error')
        ...
        |diksha_nishtha_consumption-by-course.json|
        |diksha_nishtha_consumption-by-district.json|
        |diksha_nishtha_percentage-enrollment-certification.json|
        |diksha_nishtha_program-started.json|
        |diksha_nishtha_tot-courses-medium.json|
